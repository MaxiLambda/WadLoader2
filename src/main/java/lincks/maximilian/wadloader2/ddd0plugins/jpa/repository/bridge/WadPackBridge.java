...
public class WadPackBridge extends AbstractBridge<WadPack, WadPackName, JpaRepository<WadPack, WadPackName>> implements WadPackReadWriteRepo {
    ...
    @Override
    public void deleteByName(WadPackName name){
        ((WadPackSpringRepo) repo).deleteByName(name);
    }

    @Override
    public Optional<WadPack> findByName(WadPackName name) {
        return ((WadPackSpringRepo) repo).findByName(name);
    }
}
